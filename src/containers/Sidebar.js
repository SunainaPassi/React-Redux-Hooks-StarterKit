...
const Sidebar = ({}) => {
  ...
  const { userDisplayName, roleName, toolTipRoleName } = JSON.parse(
  ...
  const handleToggleMenu = ({ menu, subMenu }) => {
    if (subMenu) {
      ...
      setToggleMenu({
        [menu.menuDisplayName]: true,
        [subMenu.menuDisplayName]: true,
      });
    } else {
      ...
      setShowSubMenu({
        [menu.menuDisplayName]: !showSubMenu[menu.menuDisplayName],
      });
    }
  };
  return (
              ...
              <div className="userProfileContainer">
                <div className="media">
                  ...
                  <div className="media-body">
                    ...
                    <div className="designation" title={toolTipRoleName}>
                      {roleName}
                    </div>
                  </div>
                </div>
            ...
            {menuList.map((menu) => {
              if (!menu.subMenus.length) {
                return (
                  <a
                    className={`nav-link ${
                      toggleMenu[menu.menuDisplayName] && "active"
                    }`}
                ...
                return (
                  <Fragment key={menu.menuid}>
                    <a
                      className={`nav-link ${
                        toggleMenu[menu.menuDisplayName] && "active "
                      }${!showSubMenu[menu.menuDisplayName] && " collapsed"}`}
                      href="javascript:void(0)"
                      onClick={() => {
                        handleToggleMenu({ menu });
                      }}
                    ...
                    <div
                      className={`${
                        showSubMenu[menu.menuDisplayName] ? "show" : "collapse"
                      }`}
                      id="collapseLayouts"
                    >
                      <nav className="sb-sidenav-menu-nested nav">
                        {menu.subMenus.map((subMenu) => {
                          return (
                            <a
                              className={`nav-link ${
                                toggleMenu[subMenu.menuDisplayName] && "active"
                              }`}
                              href="javascript:void(0)"
                              key={subMenu.menuid}
                              onClick={() => {
                                handleToggleMenu({ menu, subMenu });
                              }}
                            >
